...
@Configuration
@EnableCaching
class CacheConfiguration(private val env: Environment) : DisposableBean {
    ...
    fun hazelcastInstance(jHipsterProperties: JHipsterProperties): HazelcastInstance {
        ...
        val hazelCastInstance = Hazelcast.getHazelcastInstanceByName("kotlinWebBasic")
        if (hazelCastInstance != null) {
            log.debug("Hazelcast already initialized")
            return hazelCastInstance
        }
        val config = Config()
        config.instanceName = "kotlinWebBasic"
